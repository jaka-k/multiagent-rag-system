    ...
    with zipfile.ZipFile(file_path, 'r') as zip_ref:
        ...
        if toc_content:
            ...
            for play_order in range(1, max_play_order + 1):
                entry = next((e for e in toc_entries if e['playOrder'] == play_order), None)
                next_entry = next((e for e in toc_entries if e['playOrder'] == play_order + 1), None)
                if entry:
                    ...
                    if matched_file:
                        ...
                        if file_content:
                            ...
                            chapter_text = extract_text_from_fragment(chapter_soup, fragment_id, next_fragment_id)
                            chapters.append({
            ...
            for file_name in file_list:
                if file_name.endswith('.html') or file_name.endswith('.xhtml') or file_name.endswith('.htm'):
...
def extract_text_from_fragment(soup, fragment_id, next_fragment_id):
    ...
    for element in start_tag.next_elements:
        ...
        elif element.name in ['p', 'div', 'span', 'pre', 'code', 'h1', 'h2', 'h3', 'h4', 'h5', 'h6']:
